...
function initUDReinigung() {
  ...
  function updateButtonProgress(checklisten, done, total) {
    ...
    if (circle) {
      ...
      if (percent === 0) circle.style.stroke = "#dcdcdc";else if (percent < 50) circle.style.stroke = "#f5b700";else circle.style.stroke = "#11863a";
    }
  }
  ...
  function renderReinigungUI(data, modal, date) {
    ...
    function renderSidebar() {
      ...
      for (const [bereich, aufgaben] of Object.entries(checklisten)) {
        ...
        if (done === total && total > 0) status.classList.add("done");else if (done > 0) status.classList.add("partial");
  ...
  async function saveReinigung(postId, checklisten, container, date, showToastMsg = true) {
    const bemerkungen = container.querySelector("#ud-reinigung-bemerkungen")?.value.trim() || "";
    try {
